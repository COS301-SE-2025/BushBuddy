...
describe('Testing postingRepository', () => {
	...
	describe('fetchAllUserPosts', () => {
		test('should return user posts', async () => {
			...
			expect(db.query).toHaveBeenCalledWith('SELECT * FROM posts WHERE user_id = $1 ORDER BY created_at DESC;', [99]);
		...
		test('should throw error when DB fails', async () => {
			...
			await expect(postingRepository.fetchAllUserPosts(1)).rejects.toThrow('Error fetching all posts: DB error');
		});
	});
	...
	describe('fetchPost', () => {
		...
		test('should throw error when DB fails', async () => {
			...
			await expect(postingRepository.fetchPost(1)).rejects.toThrow('Error fetching post: DB error');
		});
	});
	...
	describe('likePost', () => {
		...
		test('should throw error when DB fails', async () => {
			...
			await expect(postingRepository.likePost(1, 1)).rejects.toThrow('Error adding like to post: DB error');
		});
	});

	describe('addComment', () => {
		test('should insert comment and return rows', async () => {
			db.query.mockResolvedValueOnce({ rows: [] }).mockResolvedValueOnce({ rows: [{ id: 1, comment_text: 'hello' }] });
		...
		test('should throw error when DB fails', async () => {
			...
			await expect(postingRepository.addComment({ user_id: 1, post_id: 1, comment_text: 'fail' })).rejects.toThrow(
				'Error adding comment to post: DB error'
			);
		});
	});
	...
	describe('fetchPostImage', () => {
		...
		test('fetchPostImage should throw error when s3 fails', async () => {
			...
			await expect(postingRepository.fetchPostImage('bad-key')).rejects.toThrow('Error fetching post image');
		});
	});
