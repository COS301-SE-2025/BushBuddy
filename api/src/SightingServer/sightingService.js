...
async function findAnimalIdByName(animalName) {
    try {
        const res = await db.query(
            'SELECT id FROM animals WHERE name LIKE $1 OR name LIKE $2',
...
async function createSighting(user_id, file, geolocation) {
    ...
    try {
        ...
        if (error.message !== 'Error uploading file' && error.message !== 'Error adding sighting to DB')
            console.error(error);
...
/**
 * NEW FUNCTION: Retrieves a single sighting from the database by its ID.
...
async function getSightingById(sightingId) {
    try {
        const res = await db.query('SELECT * FROM identifications WHERE id = $1', [sightingId]);
        if (res.rows.length > 0) {
            return res.rows[0];
        }
        return null;
        ...
        throw new Error('Failed to retrieve sighting');
    }
}
