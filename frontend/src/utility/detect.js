...
function postprocess(flatOutput, classThreshold, modelWidth, modelHeight) {
  ...
  flatOutput.forEach((row) => {
    ...
    /*console.log(
      "Logits min/max:",
      Math.min(...classLogits),
      Math.max(...classLogits)
    );*/

    const classProbs = classLogits.map(sigmoid);
    const maxClassProb = Math.max(...classProbs);
    const classIndex = classProbs.indexOf(maxClassProb);

    if (maxClassProb > classThreshold) {
