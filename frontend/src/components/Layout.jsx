...
function Layout() {
	...
	const handleUpdateSetting = async (setting) => {
		...
		switch (setting) {
			case 'THEME':
				preferences.theme = darkMode ? 'light' : 'dark';
				setDarkMode((dm) => !dm);
				break;
			case 'NOTIFICATIONS':
				...
				break;
			case 'LOCATION':
				preferences.location = !locationTracking;
				setLocationTracking((loc) => !loc);
				break;
			default:
				return;
		}
		...
		<div className="layout-wrapper">
			{showNavAndHead && (
				<Navbar fixed="top" className="header">
					<img src={require('../assets/BushBuddy.webp')} alt="App Icon" className="header-icon" />
					<Navbar.Brand className="header-brand">{getHeaderText()}</Navbar.Brand>

					{showSettingsIcon ? (
						<div className="header-settings-icon" onClick={() => setShowSettings(true)}>
			...
			{showNavAndHead && (
				<Navbar className="footer">
					...
					<Button
						className={`nav-button-profile ${path === '/profile' ? 'active' : ''}`}
			...
			<SettingsModal
				...
				darkMode={darkMode}
				onToggleDarkMode={() => handleUpdateSetting('THEME')}
				notifications={notifications}
				onToggleNotifications={() => handleUpdateSetting('NOTIFICATIONS')}
				locationTracking={locationTracking}
				onToggleLocation={() => handleUpdateSetting('LOCATION')}
			/>
			<HelpModal show={showHelp} onClose={() => setShowHelp(false)} helpText={helpText} />
		</div>
	);
}
